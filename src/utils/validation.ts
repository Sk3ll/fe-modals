...
import { IChild, IContactDetails, IRelative, Phone, IChildren, IRelatives, TypeRelatives } from './types';
...
// @ts-ignore regression bug in 0.32.9 https://github.com/jquense/yup/issues/1389
export const childrenSchema: yup.SchemaOf<IChildren> = yup.object().shape({
  children: yup.array().of<yup.SchemaOf<IChild>>(yup.object({
...
// @ts-ignore regression bug in 0.32.9 https://github.com/jquense/yup/issues/1389
export const relativesSchema: yup.SchemaOf<IRelatives> = yup.object().shape({
  relatives: yup.array().of<yup.SchemaOf<IRelative>>(yup.object({
    fullName: yup.string().required('Name is required').max(24, 'Name must be between 1 and 24 characters long'),
